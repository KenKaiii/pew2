...
- **The transcript is a FlashList v2 (`ui/ChatThread.tsx`) and owns its own scroll position.** `maintainVisibleContentPosition={{ startRenderingFromBottom: true, autoscrollToBottomThreshold: 0.2 }}` is the whole mechanism: the first painted frame is already at the newest message, a row that grows mid-stream never pushes the rows above it, and an append is followed only while the reader is near the end. Never scroll imperatively from an `onLayout` or `onContentSizeChange` — a state-driven scroll racing native repositioning is exactly the jump this replaced; the one legitimate `scrollToEnd` is the jump-to-latest button, driven by a tap. The contract that keeps it honest: `keyExtractor` is the stable turn id, every prop passed to `ChatThread` is memo-stable, and the list is remounted by `key={sessionId}` so `startRenderingFromBottom` re-arms per transcript. Cells recycle, so a mount no longer means "a new message arrived" — `Turn` is memoized and deliberately has no per-mount fade, and the streaming dots are a `ListFooterComponent` rather than a sibling inside the last row.
